        ...
        with open(path + "/" + label_names[index], "r") as fileref:
            ...
            for i in fileref:
                ...
                if product_type_id == int(i.split()[1]):
                    ...
                    helper_ID_minus_1 = int(i.split()[2]) - 1
                    names[helper_ID_minus_1] = i.split()[0]
                    # names[i.split()[2]] = i.split()[0]
                    st = " ".join(i.split()[2:])
                else:
                    if not st: # if the string is not empty
...
def update_label_id_and_names(label_names, path, product_type_id, product_type_list_names=[]):
    # Initialize an empty string to store the new label content
    st = ""
    names = {}

    # Create a reversed dictionary that maps numbers to names
    product_inverse = {value: key for key, value in products.items()}

    # save all groups in list
    all_IDS = []
    all_IDS.append(product_type_id)
    for element_name in product_type_list_names:
        all_IDS.append(products[element_name])

    all_groups = []
    for elem_ID in all_IDS:
        elem_name =  product_inverse.get(elem_ID)
        all_groups.append(elem_name)
    ...
    for index in range(len(label_names)):
        ...
        with open(path + "/" + label_names[index], "r") as fileref:
            ...
            for i in fileref:
                # Check if the second element in the line matches the product_type_id
                if product_type_id == int(i.split()[1]):
                    # Find product name from product_type_id
                    product_name = product_inverse.get(product_type_id)
                    # Get new ID by looking for product name in newLabel_from0 list
                    newID = newLabel_from0.get(product_name)
...
def create_yolo_training_config(folder_path, product_specific=False, product_type_id=None, product_type_list_names=None):
    ...
    if product_type_list_names == None:
    ...
    elif product_type_list_names != None and product_type_id != None:
        names = update_label_id_and_names(labels_names_val, labels_val_dir, product_type_id, product_type_list_names)
        names = update_label_id_and_names(labels_names_train, labels_train_dir, product_type_id, product_type_list_names)
    else:
        # Update label IDs for validation and training sets
...
create_yolo_training_config("C:/Users/Liva Ausma/Documents/WORK/DFKI/DVC/AI/training_folder/dataset", product_specific=True, product_type_id=2)
# create_yolo_training_config("C:/Users/Liva Ausma/Documents/WORK/DFKI/DVC/AI/training_folder/dataset", product_specific=True, product_type_id=1, product_type_list_names=["Toilet"])
